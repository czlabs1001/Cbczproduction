	...
	class sw_woo_tab_cat_slider_widget extends WP_Widget {
			  ...
			  "params" => array(
				 ...
				 array(
				  ...
				  array(
					...
					"param_name" => "layout",
					"admin_label" => true,
					"value" => array( 'Layout Default' => 'default', 'Layout 1' => 'layout1', 'Layout 3'=>'layout3', 'Layout 4'=>'layout4', 'Layout Mobile'=>'layout2' ),
		...
		**/
										...
										<?php
						              	...
						              	if($stock_status = $product->get_stock_status()){
							            	...
							            	if($stock_status == 'onbackorder'){
												...
												echo '<div style="color:#77a464;font-weight:bolder;"><center> In Stock </center></div>';
		...
		public function widget( $args, $instance ) {
			...
			if ( $tpl = sw_override_check( 'sw-woo-tab-category-slider', $instance['layout'] ) ){ 
		...
		function update( $new_instance, $old_instance ) {
			...
			if ( array_key_exists('img_banners', $new_instance) ){
				$instance['img_banners'] = intval( $new_instance['img_banners'] );
			}
		...
		public function form( $instance ) {
			...
			$img_banners     		= isset( $instance['img_banners'] ) 	? intval($instance['img_banners']) : '';
			...
			<p>
				...
				<input class="widefat" id="<?php echo $this->get_field_id('img_banners'); ?>" name="<?php echo $this->get_field_name('img_banners'); ?>"
					type="text"	value="<?php echo esc_attr($img_banners); ?>" />
			</p>
			<p>
			...
			<p>
				...
				<select class="widefat"
					...
					<option value="theme3" <?php if ($layout=='theme2'){?> selected="selected"
					<?php } ?>>
					...
					<option value="tab_list_product.php" <?php if ($layout=='tab_list_product.php'){?> selected="selected"
					<?php } ?>>
						<?php _e('Tab List Product', 'sw_woocommerce')?>
					</option>	
				</select>
			</p>               
		<?php
