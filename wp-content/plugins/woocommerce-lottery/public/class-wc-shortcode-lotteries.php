...
class WC_Shortcode_Lotteries {
	 ...
	 */
	public function __construct( $attributes = array(), $type = 'lotteries' ) {
		$this->type       = $type;
	 ...
	 * @return string
	 */
	public function get_content_lotteries_winners() {
		return $this->product_loop_lotteries_winners();
	}
	...
	protected function parse_attributes( $attributes ) {
		...
		$attributes = shortcode_atts(
			array(
				...
				'lottery_status' => '',      // Should shortcode output be cached.
	...
	protected function parse_query_args() {
		...
		$this->set_lotteries_query_args( $query_args );
		$this->set_lotteries_status_query_args( $query_args );
		$this->set_lotteries_future_query_args( $query_args );
		$query_args['is_lottery_archive'] = true;

		$query_args = apply_filters( 'woocommerce_shortcode_lotteries_query', $query_args, $this->attributes, $this->type );
	 ...
	 * @param array $query_args Query args.
	 */
	protected function set_lotteries_winners_query_args( &$query_args ) {
		$query_args['meta_query'][] = array(
			'key'     => '_lottery_closed',
			'value'   => '2'
		);
	...
	/**
	 * Set lotteries query args.
	 *
	 * @since 3.2.0
	 * @param array $query_args Query args.
	 */
	protected function set_lotteries_query_args( &$query_args ) {
	 ...
	 * @param array $query_args Query args.
	 */
	protected function set_lotteries_status_query_args( &$query_args ) {

		if ( ! empty( $this->attributes['lottery_status'] ) ) {

				if ( $this->attributes['lottery_status'] === 'all' ){

					$query_args['show_past_lottery'] = true;
					$query_args['show_future_lotteries'] = true;

				} elseif( $this->attributes['lottery_status'] === 'finished' ) {
				...
				} elseif( $this->attributes['lottery_status'] === 'future' ) {
			...
			} elseif($this->attributes['lottery_status'] === 'active'){
				$query_args['show_future_lotteries'] = false;
				$query_args['show_past_lottery'] = false;
			}
	 ...
	 * @param array $query_args Query args.
	 */
	protected function set_lotteries_future_query_args( &$query_args ) {
	 ...
	 * @param array $query_args Query args.
	 */
	protected function set_my_winned_lotteries_query_args( &$query_args ) {
	 ...
	 * @return string
	 */
	protected function product_loop_lotteries_winners() {
		$columns  = absint( $this->attributes['columns'] );
		$classes  = $this->get_wrapper_classes( $columns );
		$products = $this->get_query_results();
		...
		if ( $products && $products->ids ) {
			...
			foreach ( $products->ids as $product_id ) {
				...
				if( $product ) {
					?>
					<tr>
						<td><?php echo $product->get_lottery_dates_to() ?> </td>
						<td><a href="<?php echo esc_url( get_permalink( $product_id ) ); ?>" rel="bookmark"><?php echo get_the_title( $product_id ); ?></a></td>
						<td>
							<?php $lottery_winers = get_post_meta($product_id, '_lottery_winners');?>
							...
							<?php } else {
								...
								<?php echo  get_userdata($lottery_winers[0])->display_name; ?>
							<?php } ?>
						<?php } ?>
			...
			wc_get_template( 'loop/no-lotteries-winners-found.php' );
		}
