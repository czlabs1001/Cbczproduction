...
class WC_LD_License_Codes_Pages {
		...
		if ( isset( $_REQUEST['nonce'] ) && wp_verify_nonce( $_REQUEST['nonce'], basename( __FILE__ ) ) ) {
			...
			if ( $item_valid === true ) {
				if ( $item['id'] == 0 ) {
					...
					if ( ! empty( $item['license_code1'] ) && ! empty( $item['license_code1'][0] ) || (!empty( $item['license_code1'][4] )) ) {
					 	$import_data['code1'] = $item['license_code1'];
					}
					if ( ! empty( $item['license_code2'] ) && ! empty( $item['license_code2'][0] ) || (!empty( $item['license_code2'][4] ))) {
					 	$import_data['code2'] = $item['license_code2'];
					}
					if ( ! empty( $item['license_code3'] ) && ! empty( $item['license_code3'][0] ) || (!empty( $item['license_code3'][4] ))) {
					 	$import_data['code3'] = $item['license_code3'];
					}
					if ( ! empty( $item['license_code4'] ) && ! empty( $item['license_code4'][0] ) || (!empty( $item['license_code4'][4] ))) {
					 	$import_data['code4'] = $item['license_code4'];
					}
					if ( ! empty( $item['license_code5'] ) && ! empty( $item['license_code5'][0] ) || (!empty( $item['license_code5'][4] ))) {
					 	$import_data['code5'] = $item['license_code5'];
					}
					$query = "INSERT INTO {$wpdb->wc_ld_license_codes} (product_id,
							  license_code1, license_code2, license_code3, license_code4,license_code5, creation_date) VALUES ";

					foreach ( $import_data as $key => $value ) {

						array_push( $values, absint( $item['product_id'] ), $value[0], $value[1], $value[2], $value[3],$value[4],
							$current_time );
						$place_holders[] = "(%d, %s, %s, %s, %s, %s,%s)";
					}
					...
					if ( $result ) {
						update_post_meta($item['product_id'] , '_manage_stock','yes');
					...
					$update_item = array(
						...
						'license_code5'   => $item['license_code1'][4],
		...
		<table cellspacing="2" cellpadding="5" style="width: 100%;" class="form-table">
			...
			<tr class="form-field">
				<th valign="top" scope="row">
					...
					<label for="product_id"><?php _e( 'Product or Variation', 'highthemes' ) ?></label>
				</th>
				<td>
					<?php
					$args = array(
					...
					echo '<select class="select2-license-delivery" id="product_id" name="product_id" class="code" required>';
			...
			<tr class="form-field">
				...
				<td>
					<input id="license_code1" name="license_code1[]" type="text" style="width: 95%"
					       ...
					       placeholder="<?php _e( 'Main Field', 'highthemes' ) ?>">
				</td>
			</tr>
			<tr class="form-field">
				<th valign="top" scope="row">
            ...
            <tr class="form-field">
					<th valign="top" scope="row">
						<label for=""><?php _e( 'File Upload', 'highthemes' ) ?></label>
					</th>
					<td>
					    <div class="wp_berry_file_upload">
					        <div class="berry_upload_file">
					            <span class="upload dashicons dashicons-upload"></span>
					            <span class="remove dashicons dashicons-dismiss"></span>
					            <span class="bimg">
					            <?php if(!empty($item['license_code5']) && is_numeric($item['license_code5'])){
					            echo '<img src="'.wp_get_attachment_url($item['license_code5']).'" alt="no file">';
        					           } ?>
					            </span>
					       </div>
					        <input type="hidden" name="license_code1[]" value="">
					    </div>
					</td>
				</tr>
				...
				<tr class="form-field">
					...
					<td>
					    <div class="wp_berry_file_upload">
					        ...
					        <input type="hidden" name="license_code2[]" value="">
					    </div>
					</td>
				</tr>
				<tr>
				...
				<tr class="form-field">
					...
					<td>
					    <div class="wp_berry_file_upload">
					        ...
					        <input type="hidden" name="license_code3[]" value="">
					    </div>
					</td>
				</tr>
				<tr>
				...
				<tr class="form-field">
					...
					<td>
					    <div class="wp_berry_file_upload">
					        ...
					        <input type="hidden" name="license_code4[]" value="">
					    </div>
					</td>
				</tr>
				<tr>
                ...
                <tr class="form-field">
					...
					<td>
					    <div class="wp_berry_file_upload">
					        ...
					        <input type="hidden" name="license_code5[]" value="">
					    </div>
					</td>
				</tr>
			<?php endif; ?>
	...
	function license_code_validate_field( $item ) {
		...
		if ( trim( $item['license_code1'][0] ) =='' && empty($item['license_code1'][4]) ) {
			$messages[] = __( 'You must fill the first field or image field of each code', 'highthemes' );
		}
