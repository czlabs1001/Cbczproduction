		...
		if ( isset( $_REQUEST['nonce'] ) && wp_verify_nonce( $_REQUEST['nonce'], basename( __FILE__ ) ) ) {
			...
			if ( ! empty( $_REQUEST['do_import'] ) && $_REQUEST['do_import'] == 1 && ! empty( $_REQUEST['product_id'] ) && ! empty( $_REQUEST['csv_data'] ) ) {
				...
				if ( ! empty( $import_data ) ) {
					...
					$query = "INSERT INTO {$wpdb->wc_ld_license_codes} (product_id, license_code1, license_code2, license_code3, license_code4, license_code5, creation_date) VALUES ";
					...
					foreach ( $import_data as $key => $value ) {
						...
						$value[4] = isset( $value[4] ) ? trim( $value[4] ) : '';

						array_push( $values, $product_id, $value[0], $value[1], $value[2], $value[3],$value[4], $current_time );
						...
						if ( in_array( $uploaded_file_type, $allowed_file_types ) ) {
							...
							if ( ( $handle = fopen( $csv_file, "r" ) ) !== false ) {
								while ( ( $data = fgetcsv( $handle, null, "," ) ) !== false ) {
									...
									if ( count( $data ) > 5 || count( $data ) < 1 || ! isset( $data[0] ) || trim( $data[0] ) == '' ) {
										continue;
									}
									$csv_data[] = $data;
								}
			...
			<?php else: ?>
				...
				<form id="form" method="POST" enctype="multipart/form-data">
					...
					<div style="max-width: 100%;" class="card ">
						...
						<p><?php _e('As each product license code can contain up to 5 fields, you must provide at least 1 and maximum
							5 columns of data in each row. The first column must be always filled with data.', 'highthemes');?></p>
	...
	public function display_csv_preview( $item ) {
		...
		<table class="widefat">
			...
			<tr>
				...
				<th><strong><?php _e( 'Field #5', 'highthemes' ); ?></strong></th>
			</tr>
			</thead>
			...
			foreach ( $item as $value ): ?>
				...
				<tr>
                    ...
                    <td scope="row"><?php echo isset( $value[4] ) ? esc_html($value[4]) : ''; ?></td>
				</tr>
				<?php if ( $i == 10 ) {
					break;
				}
		...
		<table cellspacing="2" cellpadding="5" style="width: 100%;" class="form-table">
			...
			<tr class="form-field">
				...
				<td>
					...
					echo '<select class="select2-license-delivery" id="product_id" name="product_id" class="code">';
