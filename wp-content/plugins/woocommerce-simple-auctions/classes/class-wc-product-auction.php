...
class WC_Product_Auction extends WC_Product {
    ...
    public function get_id() {
        return $this->get_main_wpml_product_id();
    }
	...
	function is_started() {
        ...
        if ( $this->get_auction_has_started() === '1' ) return TRUE;
		...
		if ( $this->get_auction_dates_from() != false ){
			...
			if ( $date1 < $date2 ){
				...
				do_action( 'woocommerce_simple_auction_not_started', $id );
		...
		if ( !empty($this->get_auction_closed()) ){
			...
			if ($this->is_finished() && $this->is_started() ){
				...
				$result = $wpdb->get_var(
					...
					"	SELECT meta_value
						FROM $wpdb->postmeta
						WHERE meta_key = '_auction_closed'
						AND post_id = %d ",
						$id
					)
				);
	...
	function auction_history( $datefrom = FALSE, $user_id = FALSE ) {
		...
		$history = wp_cache_get( 'wsa_auction_history' . $datefrom . $user_id . $id  );
		if ( false === $history ) {
			...
			wp_cache_set( 'wsa_auction_history' . $datefrom . $user_id . $id, $history, 'wsa_cache' );
		}
		return $history;
	}
    ...
    function get_main_wpml_product_id(){
        ...
        elseif (function_exists('icl_object_id') && method_exists($sitepress, 'get_default_language')) { // WPML
        	global $wpdb;
        	$id = wp_cache_get( 'wsa_wpml_product_id' . $this->id, 'wsa_cache');
			if ( false === $id ) {
				$id = $wpdb->get_var(
					...
					WHERE trans1.element_id = ' . intval( $this->id ) . '
					AND trans2.source_language_code IS NULL');
				wp_cache_set( 'wsa_wpml_product_id' . $this->id, $id, 'wsa_cache');
			}
    ...
    public function is_user_biding( $auction_id, $user_ID = false ){
    	...
    	global $wpdb;
    	$bid_count = wp_cache_get( 'wsa_is_user_biding' . $auction_id . $user_ID );
		if ( false === $bid_count ) {
			...
			wp_cache_set( 'wsa_is_user_biding' . $auction_id . $user_ID, $bid_count, 'wsa_cache');
		}
	...
	public function get_user_max_bid( $auction_id , $user_ID = false ){
    	...
    	$maxbid = wp_cache_get( 'wsa_get_user_max_bid' . $auction_id . $user_ID );
		if ( false === $maxbid ) {
			...
			wp_cache_set( 'wsa_get_user_max_bid' . $auction_id . $user_ID, $maxbid, 'wsa_cache' );
		}
	...
	/**
	 * Get auction biddders count .
	 *
	 * @since 1.1
	 * @param  string $context
	 * @return string
	 */
	public function get_auction_bids_count( $context = 'view') {
		...
		$relisteddate = get_post_meta( $id, '_auction_relisted', true );
		$rowcount = wp_cache_get( 'get_auction_bids_count' .$id , 'wsa' );
		if ( false === $rowcount || 'edit' === $context ) {
			if ( $relisteddate ) {
				...
				$rowcount = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(1) FROM " .$wpdb -> prefix. "simple_auction_log WHERE auction_id = %d", $id )	);
			}
			wp_cache_set( 'get_auction_bids_count' . $id, $rowcount, 'wsa' );
		}
	 ...
	 * @return string
	 */
	public function get_auction_biders_count( $context = 'view') {
		...
		$relisteddate = get_post_meta( $id, '_auction_relisted', true );
		$rowcount = wp_cache_get( 'get_auction_biders_count' .$id , 'wsa' );
		...
		if ( false === $rowcount || 'edit' === $context ) {
			if ( $relisteddate ) {
				...
				$rowcount = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT( DISTINCT( userid ) ) FROM " .$wpdb -> prefix. "simple_auction_log WHERE auction_id = %d ", $id )	);
			}

			wp_cache_set( 'get_auction_biders_count' . $id, $rowcount, 'wsa' );
		}
