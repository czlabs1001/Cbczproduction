...
class WC_Shortcode_Simple_Auctions {
	...
	protected function get_query_results() {
		...
		$current_language= apply_filters( 'wpml_current_language', NULL );
		$transient_name = $transient_name . $current_language;
		...
		if ( isset( $transient_value['value'], $transient_value['version'] ) && $transient_value['version'] === $transient_version ) {
			...
			if ( $current_language && ! empty( wp_parse_id_list( $query->posts ) ) ) {
					$post_ids = wp_parse_id_list( $query->posts );
					$query_args = array('posts_per_page' => $this->query_args['posts_per_page'] , 'no_found_rows' => 1, 'post_status' => 'publish', 'post_type' => 'product', 'post__in' => $post_ids, 'orderby' => 'post__in');
					$query_args['is_auction_archive'] = $this->query_args['is_auction_archive'];
					$query_args['auction_arhive'] = $this->query_args['auction_arhive'];
					if ( isset( $this->query_args['show_future_auctions'] ) ){
						$query_args [ 'show_future_auctions' ] = $this->query_args['show_future_auctions'];
					}
					if ( isset( $this->query_args['show_past_auctions'] ) ){
						$query_args [ 'show_past_auctions' ] = $this->query_args['show_past_auctions'];
					}
