	...
	if ( ! class_exists( 'WooCommerce_simple_auction' ) ) {
		class WooCommerce_simple_auction {
			...
			public function init() {
				...
				add_action( 'woocommerce_order_status_processing', array( $this, 'auction_order' ), 10, 2 );
				add_action( 'woocommerce_order_status_processing', array( $this, 'auction_payed' ), 12, 1 );
				...
				if ( is_admin() ) {
					...
					add_action( 'admin_menu', array( $this, 'add_auction_winners_page' ) );
					...
					if ( isset($_GET['action'] ) && $_GET['action'] == 'download_winners_csv' )  {
						add_action( 'admin_init',array( $this, 'csv_export_winners') ) ;
					}
				}
				...
				if ( function_exists( 'icl_object_id' ) && is_object( $sitepress ) && method_exists( $sitepress, 'get_default_language' ) ) {
					...
					add_action( 'woocommerce_simple_auction_not_started', array( $this, 'sync_future_metadata_wpml' ), 85 );
					add_action( 'woocommerce_simple_auction_started', array( $this, 'sync_future_metadata_wpml_started' ), 85 );
			...
			public function includes() {
				...
				require_once 'classes/wc-simple-auction-winners-list.php';
			...
			public function admin_enqueue_script( $hook ) {
				...
				global $post_type;
				if ( $hook == 'post-new.php' || $hook == 'post.php' || $hook == 'woocommerce_page_auctions-activity' || $hook == 'woocommerce_page_auctions-winners' ) {
					if ( 'product' == get_post_type() || $hook == 'woocommerce_page_auctions-activity' || $hook == 'woocommerce_page_auctions-winners' ) {
						$params = array(
				...
				if ( ! is_admin() ) {
					...
					if ( ! empty( $_GET['pay-auction'] ) ) {
						...
						if ( ! is_user_logged_in() ) {
							header( 'Location: ' .  add_query_arg ('wsa_redirect_pay', $product_id, get_permalink( wc_get_page_id( 'myaccount' ) ) ));
							exit;
						}
...
?>
				...
				if ( ! empty( $order ) && $order ) {
					...
					if ( ! empty( $order_items ) && $order_items ) {
						...
						foreach ( $order_items as $item_id => $item ) {
							...
							if ( ! empty( $product_data ) && method_exists( $product_data, 'get_type' ) && $product_data->get_type() == 'auction' ) {
				...
				if ( ! empty( $all_posts ) ) {
					foreach ( $all_posts as $key => $translatedpost ) {
						...
						if ( isset( $meta_values['_auction_closed'][0] ) ) {
							...
							delete_post_meta( $translatedpost->element_id, '_auction_closed' );
						}
						...
						if ( isset( $meta_values['_auction_fail_reason'][0] ) ) {
							...
							delete_post_meta( $translatedpost->element_id, '_auction_fail_reason' );
						}
			 ...
			 *
			 */
			public function sync_future_metadata_wpml( $product_id ) {
				...
				if ( ! empty( $all_posts ) ) {
					foreach ( $all_posts as $key => $translatedpost ) {
						update_post_meta( $translatedpost->element_id, '_auction_started', '0' );
					}
				}
			}
			/**
			 ...
			 *
			 */
			public function sync_future_metadata_wpml_started( $product_id ) {
				...
				if ( ! empty( $all_posts ) ) {
					foreach ( $all_posts as $key => $translatedpost ) {
						update_post_meta( $translatedpost->element_id, '_auction_has_started', '1' );
						delete_post_meta( $translatedpost->element_id, '_auction_started');
			...
			*/
			 ...
			 *
			*/
			public function add_auction_winners_page() {

				$hook = add_submenu_page( 'woocommerce', 'Auctions winners', 'Auctions Winners', 'manage_woocommerce', 'auctions-winners', array( $this, 'create_auction_winnes_admin_page' ) );
			...
			*/
			...
			/**
			 *
			 * Auction winners page in wp-admin
			 *
			 * @access public
			 * @param void
			 * @return void
			 *
			*/
			public function create_auction_winnes_admin_page() {
				...
				printf( '<div class="wrap" id="wpse-list-table"><h2>%s</h2>', esc_html__( 'Auction Winners', 'wc_simple_auctions' ) );
				echo '<form id="wpse-list-table-form" method="get">';

				$wp_list_table = new Wc_Simple_Auctions_Winners_List_Table();
				$wp_list_table->prepare_items();
				$wp_list_table->search_box( 'search', 'search_id' );
				foreach ( $_GET as $key => $value ) {
					if ( 's' !== $key ) {
				...
				$wp_list_table->datepicker();
				$wp_list_table->display();
				echo '<a href="' . add_query_arg( array( 'action' => 'download_winners_csv', '_wpnonce' => wp_create_nonce( 'download_winners_csv' ) ) ) . '" class="page-title-action">'. __('Export to CSV','wc_simple_auctions') .'</a>';
				echo '</form>';
				echo '</div>';
				echo '<div class="clear"></div>';
			...
			*/
			function csv_export_winners() {
				...
				if ( ! wp_verify_nonce( $nonce, 'download_winners_csv' ) ) {
					die( 'Security check error' );
				}
				...
				$filename = 'auction_winners_'. time() . '.csv';

				$header_row = $wp_list_table->get_columns();
				$header_row = array_merge(array_slice($header_row, 0, 6), array('user_email'=>'Winner Email'), array_slice($header_row, 1));
				...
				foreach ( $wp_list_table->items as $item ) {
					...
					$row = array(
						...
						$product->get_sku(),
						$item['post_title'],
                		$product->get_auction_bids_count(),
                        $product->get_auction_biders_count(),
						esc_attr( $userdata->user_nicename ),
						esc_attr( $userdata->user_email ),
						$product->get_auction_current_bid(),
						$item['end_date'],
			...
			*/
			...
			public function add_redirect_previous_page() {
				...
				if ( isset($_GET['wsa_redirect_pay'] ) ) {
					echo '<input type="hidden" name="redirect" value="'. esc_attr( add_query_arg( "pay-auction", intval( $_GET['wsa_redirect_pay'] ), simple_auction_get_checkout_url() ) ) . '" >';
				}
			}
			...
			function custom_rewrite_rule() {
				...
				$trid = apply_filters( 'wpml_element_trid', NULL, $auction_page_id, 'post_page' );
				$translations = apply_filters( 'wpml_get_element_translations', NULL, $trid, 'post_page' );
				if ( $translations ) {

					foreach ($translations as $key => $value) {
						add_rewrite_rule( '^'.get_page_uri( $value->element_id ).'/?$','index.php?auction_base_page=true','top');
						add_rewrite_rule( '^'.get_page_uri( $value->element_id ).'/page/([0-9]{1,})/?$', 'index.php?auction_base_page=true&paged=$matches[1]', 'top' );
					}
			...
			public function exclusion_finished_auctions_from_categories_widget( $terms, $taxonomy, $query_vars, $term_query ){
				...
				foreach ($terms as $key => $value) {
					...
					if ( is_object( $value ) && $value->taxonomy == 'product_cat' ){
						...
						$key      = md5( wp_json_encode( $args ) ) . 'wsa';
						$products = wp_cache_get( $key );
						if ( false === $products ) {
							$products = new WP_Query( $args );
							wp_cache_set( $key, $products, 'wsa_cache', 1);
						}
	...
	add_action( 'elementor/init', 'wsa_load_elementor_support');
